...
def test_cdc_check_passes() -> None:
    out = run_cdc_check(_cfg())
    assert out["status"] == "pass", out
    assert out["crossings_total"] == 2
    assert out["by_verdict"] == {"safe": 2, "unsafe": 0, "waived": 0}


def test_cdc_check_engine_field_present() -> None:
    """Every cdc-check envelope (v1.0+) carries audit_engine."""
    out = run_cdc_check(_cfg())
